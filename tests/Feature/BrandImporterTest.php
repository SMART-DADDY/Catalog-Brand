...
function makeImporter(array $options): BrandImporter
{
    return new class(new Import(), [], $options) extends BrandImporter {
...
it('builds import completion notifications with failed rows', function (): void {
    ...
    expect(BrandImporter::getCompletedNotificationBody($import))
        ->toBe('Your brand import has completed and 8 rows imported. 2 rows failed to import.');
    ...
    expect(BrandImporter::getCompletedNotificationBody($allSuccessful))
        ->toBe('Your brand import has completed and 1 row imported.');
});
